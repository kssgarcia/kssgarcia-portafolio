...
function Transition({ children }) {
  ...
  useGSAP(() => {
    if (children.key !== displayChildren.key) {
      ...
      } else if (location.state && location.state.title) {
        setTitle(location.state.title);
      } else {
        setTitle("Main");
      }
      gsap
        .to(slideOut.current, { scaleY: 1, duration: 1, ease: "custom" })
        .then(() => {
          setDisplayChildren(children);
          gsap.to(slideOut.current, {
            scaleY: 0,
            duration: 1,
            delay: 0.4,
            ease: "custom",
    ...
    <div ref={container}>
      ...
      >
        <img
          ...
          id="svg-cursor"
          src="/kssgarcia-portafolio/cursor.svg"
          alt="Cursor"
        />
      </div>
      {displayChildren}

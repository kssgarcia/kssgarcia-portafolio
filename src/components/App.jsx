...
function App() {
  ...
  const handleImageClick = (project) => {
    ...
    navigate(`/kssgarcia-portafolio/project/${project.key}`, {
      state: { ...project, scrollY },
    });
  };

  const handlePostClick = (post) => {
    const scrollY = window.scrollY;
    navigate(`/kssgarcia-portafolio/post/${post.index}`, {
      state: { ...post, scrollY },
    });
  };
  ...
  useEffect(() => {
    ...
    axios
      .get("https://kssgarcia.github.io/BlogPersonal/blog/index.xml", {
  ...
  useGSAP(() => {
        ...
        if (config.draggable && typeof Draggable === "function") {
          ...
          draggable = Draggable.create(proxy, {
            ...
            onRelease() {
              ...
              let diffX = Math.abs(
                Math.abs(draggable.startX) - Math.abs(draggable.x),
              );
              if (draggable.getDirection("start") === "left" && diffX > 1) {
                loop.next({ duration: 0.7, ease: "back.out" });
              } else if (
                draggable.getDirection("start") === "right" &&
                diffX > 1
              ) {
                loop.previous({ duration: 0.7, ease: "back.out" });
              }
    ...
    if (!window.matchMedia("only screen and (max-width: 1000px)").matches) {
      ...
      const proj_elem = document.querySelectorAll(
        ".container-projects .title-projects",
      );
      const proj_anim = gsap.to(".container-projects .title-projects .char", {
        y: 0,
        opacity: 1,
        stagger: 0.05,
        duration: 0.5,
      ...
      const blog_elem = document.querySelectorAll(
        ".container-blog .title-blog",
      );
      const blog_anim = gsap.to(".container-blog .title-blog .char", {
        y: 0,
        opacity: 1,
        stagger: 0.05,
        duration: 0.5,
      ...
      gsap.utils
        .toArray(".container-projects .list-projects .project .project-info")
        .forEach((text) => {
          const anim = gsap.to(text.querySelectorAll(".char"), {
            opacity: 1,
            stagger: 0.01,
  ...
  return (
    <div id="container">
      <div className="front">
        <div className="front-titles title-1">PORTAFOLIO</div>
        <div className="front-titles title-2">MECHANICAL</div>
        <div className="front-titles title-3">ENGINEER</div>
        ...
        <div className="content-foot">
          <div className="social-contact">
            ...
            <div className="socials">
              <a href="https://github.com/kssgarcia" className="gihub">
                Github
              </a>
              <a
                href="https://www.instagram.com/kssgarcia7"
                className="instagram"
              >
                Instagram
              </a>
